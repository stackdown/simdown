...
class BaseService {
  ...
  updateItem(path, data, callback) {
    ...
    dbutil.get(this.db, path, (err, existing) => {
      let update = data
      let updated = extend(true, existing || {}, update)

      _this.log("Updating item", path.join('.'), update)
      dbutil.put(_this.db, path, updated, (err) => {
        callback(err, data)
      })
    })
  }

  listItems(path, respPath, params, callback) {
  ...
  setup(db, opts, callback) {
    ...
    async.forEachOf(handlers, (handler, route, next) => {
      ...
      for (var method of methods) {
        _this.app[method.toLowerCase()](fullPath, (req, res, next) => {
          handler(opts, req, res, (err) => {
            next(err)
          })
        })
      }
